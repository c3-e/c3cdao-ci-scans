...
1. **Make the repo conform.** The derived contract reads files you already
   ...
   tag and healthcheck, Dockerfiles carry the `BUILDER_IMAGE`/`RUNTIME_IMAGE`
   ARG pair, and the chart exposes readiness plus one Service-backed HTTP smoke
   target ([docs/CI-CONTRACT.md](docs/CI-CONTRACT.md)).
2. **Copy the caller.**
...
3. **Edit `with:` and pin.** Each `with:` line carries an inline provenance
   comment. Keep the job id `security-scan` — renaming it silently un-gates
